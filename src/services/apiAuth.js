...
export async function updateCurrentUser({ password, fullName, avatar, userAvatar }) {
  // If needs to update avatar , REMOVE Old AVATAR
  if (avatar !== null) {
    let oldAvatar = userAvatar.split("/").splice(-1).join('');
    await supabase.storage.from('avatars').remove(oldAvatar);
  }
  // 1.Update password Or fullName avatar
  let updateData;
